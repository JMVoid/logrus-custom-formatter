...
const (
	...
	// JMVoid custom time format
	JMVoidTimestampFormat = "2006-01-02 15:04:05"
)
...
func NewFormatter(template string, custom CustomHandlers) *CustomFormatter {
	formatter := CustomFormatter{
		...
		TimestampFormat: JMVoidTimestampFormat,
		startTime:       time.Now(),
	}
	...
	f := logrus.StandardLogger().Out.(*os.File)
	if !terminal.IsTerminal(int(f.Fd())) || (runtime.GOOS == "windows" && !WindowsNativeANSI()) {
